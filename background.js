class QuickMoveModel {
  setMailFolders(mailFolders) {
    if (mailFolders) {
      this.mailFolders = mailFolders;
    }

    this.mailFolderNames = [];
    this.mailFolderTemp = [];
    for (const folder of this.mailFolders) {
      this.mailFolderNames.push(folder.name.toLowerCase());
      this.mailFolderTemp.push(folder);
    }
    console.log("QuickMoveModel", this.mailFolderNames, this.mailFolders);

    this._setupSuffixTree();
  }

  async _setupSuffixTree() {
    // this.suffixTree = await browser.quickmove.setupMultiSuffixTree(
    //   this.mailFolderNames,
    //   this.mailFolderTemp
    // );
    // console.log(
    //   "QuickMoveModel suffixtree came back",
    //   // this.suffixTree
    //   this.suffixTree.findMatches("tufts")
    // );
  }
}
...
const handleStartup = () => {
  const quickMoveController = new QuickMoveController();
