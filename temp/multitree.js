...
State.prototype = {
    ...
    toString() {
        return '[Start: ' + this.start + ' End: ' + this.end + (this.suffix ? ' non-null suffix]' : ' null suffix]');
    },
};
 ...
 */
    ...
    dump(aState, aIndent, aKey) {
        ...
        if (aState.isImplicit) {
            let snip;
            if (aState.length > 10) {
                snip = this._str.slice(aState.start, Math.min(aState.start + 10, this._str.length)) + '...';
            } else {
                snip = this._str.slice(aState.start, Math.min(aState.end, this._str.length));
            }
            dump(aIndent + aKey + ':' + snip + '(' + aState.start + ':' + aState.end + ')\n');
        } else {
