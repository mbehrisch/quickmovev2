...
export class LegacyOptions {
    ...
    private getLegacyAccountSettings(accountInfos: AccountInfo[]): AccountSettingsArray {
        ...
        for (const accountInfo of accountInfos) {
            const accounts: Ci.nsIMsgAccount[] = iteratorUtils.fixIterator(
                mailservices.MailServices.accounts.accounts,
                Ci.nsIMsgAccount,
            );
            let account;
            ...
            if (
                settingOfAccount.bArchiveOther ||
                settingOfAccount.bArchiveMarked ||
                settingOfAccount.bArchiveTagged ||
                settingOfAccount.bArchiveUnread
            ) {
    ...
    private getInternalLegacyPrefBranch(): Ci.nsIPrefBranch {
        const prefs: Ci.nsIPrefService = Components.classes['@mozilla.org/preferences-service;1'].getService(
