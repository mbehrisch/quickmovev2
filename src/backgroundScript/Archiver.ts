...
export class Archiver {
    ...
    private async archiveFolder(folder: MailFolder, settings: AccountSettings): Promise<void> {
        try {
            ...
            log.info(
                "start searching messages to archive in folder '" +
                    folder.path +
                    "' (" +
                    folder.type +
                    ") in account '" +
            ...
            log.info(
                "message search done for '" +
                    folder.name +
                    "' in account '" +
                    mailAccount.name +
                    "' -> " +
                    messages.length +
                    ' messages found to archive',
            );

            if (messages.length > 0) {
                log.info(
                    "start real archiving of '" +
                        folder.name +
                        "' (" +
                        messages.length +
                        " messages) in account '" +
    ...
    private async searchMessagesToArchive(folder: MailFolder, settings: AccountSettings): Promise<MessageHeader[]> {
        ...
        try {
            ...
            log.errorException(
                e,
                'The exception might occur because the folder is a virtual folder... See https://bugzilla.mozilla.org/show_bug.cgi?id=1529791',
            );
            return messages;
        }
    ...
    private detectMessagesToArchive(
