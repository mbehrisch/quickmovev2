            ...
            .each((index: number, element: HTMLElement) => {
                ...
                if (accountId) {
                    settings.accountSettings[accountId] = {
                        ...
                        daysUnread: Number(
                            (getElementForAccount(accountId, 'archiveUnreadDays') as HTMLInputElement).value,
                        ),
                        bArchiveMarked: (getElementForAccount(accountId, 'archiveStarred') as HTMLInputElement).checked,
                        daysMarked: Number(
                            (getElementForAccount(accountId, 'archiveStarredDays') as HTMLInputElement).value,
                        ),
                        bArchiveTagged: (getElementForAccount(accountId, 'archiveTagged') as HTMLInputElement).checked,
                        daysTagged: Number(
                            (getElementForAccount(accountId, 'archiveTaggedDays') as HTMLInputElement).value,
                        ),
                        bArchiveOther: (getElementForAccount(accountId, 'archiveMessages') as HTMLInputElement).checked,
                        daysOther: Number(
                            (getElementForAccount(accountId, 'archiveMessagesDays') as HTMLInputElement).value,
                        ),
                    };
                }
            });
...
const restoreOptions = async () => {
    ...
    accountsSorted.forEach((accountInfos) => {
        ...
        (getElementForAccount(accountId, 'archiveUnreadDays') as HTMLInputElement).value =
            accountSetting.daysUnread.toString();
        (getElementForAccount(accountId, 'archiveStarred') as HTMLInputElement).checked = accountSetting.bArchiveMarked;
        (getElementForAccount(accountId, 'archiveStarredDays') as HTMLInputElement).value =
            accountSetting.daysMarked.toString();
        (getElementForAccount(accountId, 'archiveTagged') as HTMLInputElement).checked = accountSetting.bArchiveTagged;
        (getElementForAccount(accountId, 'archiveTaggedDays') as HTMLInputElement).value =
            accountSetting.daysTagged.toString();
        (getElementForAccount(accountId, 'archiveMessages') as HTMLInputElement).checked = accountSetting.bArchiveOther;
        (getElementForAccount(accountId, 'archiveMessagesDays') as HTMLInputElement).value =
            accountSetting.daysOther.toString();
    });
};

const getJQueryElementForAccount = (accountId: string, elementId: string) => {
    const id = elementId + '-' + accountId;
    const jQueryElem = $('#' + id);
    return jQueryElem;
};

const getElementForAccount = (accountId: string, elementId: string) =>
    getJQueryElementForAccount(accountId, elementId)[0];

const cloneTemplate = (cloneId: string, appendToId: string, accountInfo: AccountInfo) => {
